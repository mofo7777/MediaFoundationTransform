...
HRESULT CMFTSimpleDirectxAware::ProcessInput(DWORD dwInputStreamID, IMFSample* pSample, DWORD dwFlags){
	...
	IMFMediaBuffer* pOuttputBuffer = NULL;
	IDirect3DSurface9* pOuttputSurface = NULL;

	IMFMediaBuffer* pIntputBuffer = NULL;
	IDirect3DSurface9* pIntputSurface = NULL;

	try{

		IF_FAILED_THROW(hr = m_pVideoSampleAllocator->AllocateSample(&pOutputSample));
		IF_FAILED_THROW(hr = pOutputSample->GetBufferByIndex(0, &pOuttputBuffer));
		IF_FAILED_THROW(hr = MFGetService(pOuttputBuffer, MR_BUFFER_SERVICE, __uuidof(IDirect3DSurface9), reinterpret_cast<void**>(&pOuttputSurface)));

		IF_FAILED_THROW(hr = pSample->GetBufferByIndex(0, &pIntputBuffer));

		if(SUCCEEDED(hr = MFGetService(pIntputBuffer, MR_BUFFER_SERVICE, __uuidof(IDirect3DSurface9), reinterpret_cast<void**>(&pIntputSurface)))){

			IF_FAILED_THROW(hr = OnCopySurfaceToSurface(pIntputSurface, pOuttputSurface));
		}
		else{

			IF_FAILED_THROW(hr = OnCopyBufferToSurface(pIntputBuffer, pOuttputSurface));
		}
	...
	SAFE_RELEASE(pIntputSurface);
	SAFE_RELEASE(pIntputBuffer);

	SAFE_RELEASE(pOuttputSurface);
	SAFE_RELEASE(pOuttputBuffer);
